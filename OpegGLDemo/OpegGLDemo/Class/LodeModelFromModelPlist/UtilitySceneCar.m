...
@interface UtilitySceneCar()
@property (strong, nonatomic, readwrite) UtilityModel *model;
...
@end
@implementation UtilitySceneCar
...
{
    for(UtilitySceneCar *currentCar in cars)
    {
        if(currentCar != self)
        {
...
{
    self.yawRadians = UtilitySceneScalarSlowLowPassFilter(elapsed,
...
GLfloat UtilitySceneScalarFastLowPassFilter(NSTimeInterval elapsed,    // seconds elapsed since last call
...
// this function repeatedly to asymptotically return values closer
// to target: "ease in" to the target value.
GLfloat UtilitySceneScalarSlowLowPassFilter(NSTimeInterval elapsed,    // seconds elapsed since last call
                                     GLfloat target,            // target value to approach
                                     GLfloat current)           // current value
...
// to target: "ease in" to the target value.
GLKVector3 UtilitySceneVector3FastLowPassFilter(NSTimeInterval elapsed,    // seconds elapsed since last call
                                         GLKVector3 target,         // target value to approach
                                         GLKVector3 current)        // current value
{
    return GLKVector3Make(UtilitySceneScalarFastLowPassFilter(elapsed, target.x, current.x),
                          UtilitySceneScalarFastLowPassFilter(elapsed, target.y, current.y),
                          UtilitySceneScalarFastLowPassFilter(elapsed, target.z, current.z));
}
...
// Call repeatedly to asymptotically return vectors closer
// to target: "ease in" to the target value.
GLKVector3 UtilitySceneVector3SlowLowPassFilter(NSTimeInterval elapsed,    // seconds elapsed since last call
                                         GLKVector3 target,         // target value to approach
                                         GLKVector3 current)        // current value
{
    return GLKVector3Make(UtilitySceneScalarSlowLowPassFilter(elapsed, target.x, current.x),
                          UtilitySceneScalarSlowLowPassFilter(elapsed, target.y, current.y),
                          UtilitySceneScalarSlowLowPassFilter(elapsed, target.z, current.z));
}
